...
public class TieCodeIncrementalAnalyzeManager extends AsyncIncrementalAnalyzeManager<State, Long> {

    private final static int STATE_NORMAL = 0;
    private final static int STATE_INCOMPLETE_COMMENT = 1;

    private final ThreadLocal<TieCodeTextTokenizer> tokenizerProvider = new ThreadLocal<>();
    protected IdentifierAutoComplete.SyncIdentifiers identifiers = new IdentifierAutoComplete.SyncIdentifiers();

    private synchronized TieCodeTextTokenizer obtainTokenizer() {
        var res = tokenizerProvider.get();
        if (res == null) {
            res = new TieCodeTextTokenizer("");
