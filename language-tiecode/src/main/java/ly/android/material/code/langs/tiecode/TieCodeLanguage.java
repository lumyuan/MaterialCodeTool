 ...
 *
 * @author lumyuan cover Rosemoe
 */
public class TieCodeLanguage implements Language {
    ...
    private final static CodeSnippet 循环体 = CodeSnippetParser.parse("循环(${1:i}, ${2:0}, $0)\n    $0\n结束 循环");

    private IdentifierAutoComplete autoComplete;
    private final TieCodeIncrementalAnalyzeManager manager;
    private final TieCodeQuoteHandler javaQuoteHandler = new TieCodeQuoteHandler();

    public TieCodeLanguage() {
        autoComplete = new IdentifierAutoComplete(TieCodeTextTokenizer.sKeywords);
        manager = new TieCodeIncrementalAnalyzeManager();
    }
...
//        }
        if ("循环体".startsWith(prefix) && prefix.length() > 0){
            publisher.addItem(new SimpleSnippetCompletionItem("循环体", "快捷代码 - 循环体", new SnippetDescription(prefix.length(), 循环体, true)));
        }
    }
    ...
    private int getIndentAdvance(String content) {
        TieCodeTextTokenizer t = new TieCodeTextTokenizer(content);
        Tokens token;
        int advance = 0;
